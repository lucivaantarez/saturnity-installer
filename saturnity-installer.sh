...
try:
    # ── Detect GitHub Pages URL: USERNAME.github.io/REPO ──
    ...
    if m:
        ...
        links = [a["browser_download_url"] for a in assets
                 if a["name"].lower().endswith(".apk") or a["name"].lower().endswith(".apk.apk")]
        print("\n".join(links))
    else:
        # ── Fallback: scrape HTML for APK hrefs ──
        html = fetch(url)
